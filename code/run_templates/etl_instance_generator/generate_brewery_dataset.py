...
def parse_arguments():
    ...
    parser.add_argument(
        "-t", "--tables", help="Number of tables in the bar", default="10", type=int
    )
    parser.add_argument(
        "-c",
        "--customers",
    ...
    parser.add_argument(
        "-n",
        "--name",
        help="Name of the generated instance",
        default="brewery_instance",
    )
    parser.add_argument(
        "-l",
        "--locale",
        help="Localization of generated names (leave empty to use English)",
    )

    parser.add_argument(
        "--restock", help="Restock quantity for generated bar", default="30", type=int
    )
    parser.add_argument(
        "--stock", help="Stock quantity for generated bar", default="60", type=int
    )
    parser.add_argument(
        "--waiters", help="Number of waiters for generated bar", default="10", type=int
    )

    parser.add_argument(
        "--satisfaction",
        help="Initial statisfaction of generated customers",
        default="0",
        type=int,
    )
    parser.add_argument(
        "--surrounding_satisfaction",
        help="Initial surrounding statisfaction of generated customers",
        default="0",
        type=int,
    )
    parser.add_argument(
        "--thirsty",
        help="Value for the Thirsty attribute of generated customers",
        default=False,
        type=bool,
    )
...
def generate_bar_csv_file_content(bar):
    ...
    cells = [bar["id"], str(bar["waiters"]), str(bar["restock"]), str(bar["stock"])]
...
def generate_bar_to_customers_csv_file_content(bar_to_customers):
    ...
    for bar, customers in bar_to_customers.items():
        for customer in customers:
            cells = [bar, customer, bar + "_contains_" + customer]
            file_content += ",".join(cells) + "\n"
    return file_content
...
def generate_arc_to_customers_csv_file_content(customers_to_customers_links):
    ...
    for pair in customers_to_customers_links:
        cells = [pair[0], pair[1], "arc_from_" + pair[0] + "_to_" + pair[1]]
        file_content += ",".join(cells) + "\n"
        cells = [pair[1], pair[0], "arc_from_" + pair[1] + "_to_" + pair[0]]
        file_content += ",".join(cells) + "\n"
    return file_content


def generate_csv_files_content(
    bar, customers, bar_to_customers, customers_to_customers_links
):
    return {
        ...
        "arc_Satisfaction.csv": generate_arc_to_customers_csv_file_content(
            customers_to_customers_links
        ),
    }


def export_csv_files(csv_files_content, dir_path):
    if not os.path.exists(dir_path):
        os.mkdir(dir_path)
    for file_name, file_content in csv_files_content.items():
        with open(os.path.join(dir_path, file_name), "w") as writer:
            writer.write(file_content)


def generate_brewery_dataset(args, working_dir="."):
    bar_options = {
    ...
    customers_to_customers_links = generate_customers_to_customers_links(
        customer_groups
    )

    csv_files_content = generate_csv_files_content(
        bar, customers, bar_to_customers, customers_to_customers_links
    )
    export_csv_files(csv_files_content, working_dir)
    LOGGER.info(f"Instance CSV files generated in folder {working_dir}")


def main():
    args = parse_arguments()
    generate_brewery_dataset(args)
