...
def filter_customers_file(customers_file_path, thirsty_value_to_keep):
    # Read all rows and filter
    with open(customers_file_path, "r") as input_file:
        reader = csv.DictReader(input_file)
        fieldnames = reader.fieldnames
        str_value_to_keep = 'false' if thirsty_value_to_keep == 'NOT_THIRSTY' else 'true'
        filtered_rows = [row for row in reader if row[fieldnames[3]] == str_value_to_keep]
...
def main():
    ...
    thirsty_value_to_keep = parameters.get("etl_param_subdataset_filter_is_thirsty", '')
    ...
    filter_customers_file(customers_file_path, thirsty_value_to_keep)
