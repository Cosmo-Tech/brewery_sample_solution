...
def etl_sub_dataset_by_filter_multi(
    api, organization_id, parent_dataset_id, subdataset_id, workspace_id, graph_filter_list
):
...
# get nodes without label :Customer OR nodes with label:Customer but with their property id is in the list X return as n
...
# if they do their property `id` is not in list X
    ...
    edge_query = [f"MATCH (src)-[edge]->(dst) WHERE "
