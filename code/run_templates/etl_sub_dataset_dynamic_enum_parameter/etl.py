...
def filter_customers_file(customers_file_path, name_of_customer_to_keep):
    # Read all rows and filter
    with open(customers_file_path, "r") as input_file:
        reader = csv.DictReader(input_file)
        fieldnames = reader.fieldnames
        filtered_rows = [row for row in reader if row[fieldnames[0]] == name_of_customer_to_keep]
...
def main():
    ...
    name_of_customer_to_keep = parameters.get("etl_param_subdataset_filter_dynamic_customer_name", '')
    ...
    filter_customers_file(customers_file_path, name_of_customer_to_keep)
