...
def main():
    ...
    with open(
        os.path.join(os.environ.get("CSM_PARAMETERS_ABSOLUTE_PATH"), "parameters.json")
    ) as f:
    ...
    dataset_api_instance.update_dataset(
        os.environ.get("CSM_ORGANIZATION_ID"), runner_data["dataset_list"][0], dataset
    )
    ...
    dataset_api_instance.update_dataset(
        os.environ.get("CSM_ORGANIZATION_ID"),
        runner_data["dataset_list"][0],
        not_pending_dataset,
    )
